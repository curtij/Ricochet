...
def MaxTrigger(signal, stream, pulse_locations):
    ...
    mod_pulse_locations=[]
    non_unique_max_count = 0
    for k in pulse_locations:
        if k[0]+2*deltat<len(stream):
            
            short_stream=stream[k[0]:k[1]] #original pulse
            short_stream_max=np.max(short_stream) 
            #Added to standardize start time"
            indexes_found = np.where(short_stream==short_stream_max)[0]
            if len(indexes_found) > 1:
                print "Warning multiple maximums found for pulse at ", k[0]/fe, "seconds"
                non_unique_max_count+=1
            max_location = indexes_found[0]+k[0]
            new_start = max_location - int(0.25*fe)
            new_end = new_start + int(1*fe)
            
            minsep_int=stream[k[0]:k[1]+min_separation]
            if short_stream_max==np.max(minsep_int): 
#                mod_pulse_locations.append((k[0],k[0]+deltat))
                mod_pulse_locations.append( (new_start, new_end) )
...
def optimalFiltering2amp(stream, pulse_locations, J, freq,t0=-0.1,temp_1=template_slow,temp_2=template_fast, start_timestep = 0, end_timestep = -1, scan_range = 0.3, bounds = None, window = "boxcar", method = "Powell", plots = True ):
    '''    
    J is the noise PSD
...
def processStream(V, t, directory):
    ...
    for p in range(len(mod_pulse_locations)):
        if p%30==0:
            start_p = mod_pulse_locations[p][0]
            end_p = mod_pulse_locations[p][1]
            pl.plot(range(start_p, end_p ), stream_active[start_p:end_p], label='Unfiltered Pulse', color='b')
            pl.plot(range(start_p, end_p ), OF_stream[start_p:end_p], label='OF Pulse', color='r')
            pl.legend()
            pl.show()
...
def processFile(file_stream, chunk_size, start_chunk=0, end_chunk=-1):
    '''
